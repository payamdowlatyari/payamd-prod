...
const RubiksCubeModel: React.ForwardRefExoticComponent<any> = forwardRef(
  (props, ref) => {
    const ANIMATION_DURATION: number = 1.2;
    const GAP: number = 0.01;
    const RADIUS: number = 0.075;

    const mainGroupRef: React.MutableRefObject<THREE.Group | null | undefined> =
      useRef();
    const isAnimatingRef: React.MutableRefObject<boolean> = useRef(false);
    const currentRotationRef: React.MutableRefObject<number> = useRef(0);
    const lastMoveAxisRef: React.MutableRefObject<string | null> = useRef(null);
    const currentMoveRef: React.MutableRefObject<any> = useRef(null);
    const animationFrameRef: React.MutableRefObject<number | null> =
      useRef(null);
    const isMountedRef: React.MutableRefObject<boolean> = useRef(true);
    const viewportSizeRef: React.MutableRefObject<{
      width: number;
      height: number;
    }> = useRef({
    ...
    const isResizingRef: React.MutableRefObject<boolean> = useRef(false);
    const resizeTimeoutRef: React.MutableRefObject<any> = useRef(null);
...
function EnhancedSpotlight(
  ...
  const light: MutableRefObject<any> = useRef();
