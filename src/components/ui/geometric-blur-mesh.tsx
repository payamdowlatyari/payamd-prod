...
/**
 * GeometricBlurMesh component
 *
 * @param {GeometricBlurMeshProps} props - The properties for the GeometricBlurMesh component.
 */
export default function GeometricBlurMesh({
  ...
  // Randomize starting shape on mount and reset timer
  // and change shape on every 10 seconds
  useEffect(() => {
    if (!repeat) return;
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      mouseDampRef.current.x +=
        (mouseRef.current.x - mouseDampRef.current.x) *
        dampingFactor *
        deltaTime;
      mouseDampRef.current.y +=
        (mouseRef.current.y - mouseDampRef.current.y) *
        dampingFactor *
        deltaTime;
